...
class NotebookMemoryLayer:
    def __init__(
        ...
        # Configure Zep client with longer timeout (60 seconds instead of default)
    ...
    def _setup_user_and_session(self, create_new_session: bool):
        try:
            ...
            self._retry_with_backoff(ensure_user)
            
            if create_new_session:
                # Delete previous session (with retry)
                def delete_session():
                    try:
                ...
                self._retry_with_backoff(delete_session)
                ...
                self._retry_with_backoff(create_session)
            else:
                # Try to use existing session, create if doesn't exist (with retry)
                ...
                self._retry_with_backoff(get_or_create_session)
    ...
    def _retry_with_backoff(self, func, *args, max_retries: int = 3, **kwargs):
        ...
        for attempt in range(max_retries):
            try:
                ...
                error_str = str(e)
                # Check if it's a timeout error (408 status code)
                if "408" in error_str or "timeout" in error_str.lower():
                    if attempt < max_retries - 1:
                        wait_time = (2 ** attempt)  # 1s, 2s, 4s
                        logger.warning(f"Timeout error (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                        time.sleep(wait_time)
                        continue
                # If it's not a timeout or last attempt, raise
                raise
        return None
