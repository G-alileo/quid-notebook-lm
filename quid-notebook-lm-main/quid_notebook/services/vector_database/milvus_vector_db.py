...
from quid_notebook.services.embeddings.embedding_generator import EmbeddedChunk
...
class MilvusVectorDB:
    def __init__(
        self, 
        db_path: str = "./milvus_notebook_lm.db",
        collection_name: str = "notebook_documents",
    ...
    def _setup_collection(self):
        try:
            if self.client.has_collection(collection_name=self.collection_name):
                ...
                # Load collection if using cloud (required for serverless)
                if self.use_cloud:
                    try:
        ...
        try:
            if self.use_cloud:
                try:
                    ...
                    logger.warning(f"Load collection warning: {str(load_error)}")
